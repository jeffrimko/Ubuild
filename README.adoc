...
The default Ubuild script name is `_Build.py` but occasionally it might make sense to have an alternative name. The script must match the following regex: `_[A-Z][A-z0-9_-]*\.py$`

The script must also contain the following line exactly without any changes or trailing whitespace: `# -*- ubuild -*-`
